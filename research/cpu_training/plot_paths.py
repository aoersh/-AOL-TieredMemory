...
modes=['dram','direct','sync','async'] + (['demand'] if 'demand' in summary['small'] else []) + (['ranked'] if 'ranked' in summary['small'] else [])
labels={'dram':'Managed\nDRAM','direct':'Direct\nCXL','sync':'Sync\nmigrate','async':'FIFO\nasync','demand':'Backward\nordered','ranked':'Pack-time\npriority'}
for ax,shape in zip(axes[:2],('small','large')):
 ...
 ax.set_xticks(range(len(modes)),[labels[m] for m in modes])
...
for i,shape in enumerate(('small','large')):
 target='ranked' if 'ranked' in modes else 'demand' if 'demand' in modes else 'async'
 stat=summary[shape][f'paired_direct_minus_{target}']
...
axes[2].set_xlabel('Direct minus tested prefetch step time (ms)\nPositive favors async; paired 95% t CI')
